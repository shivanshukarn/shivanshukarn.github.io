...
const Sidebar = () => {
   ...
   return (
      <>
         <button data-drawer-target="sidebar-multi-level-sidebar" data-drawer-toggle="sidebar-multi-level-sidebar" aria-controls="sidebar-multi-level-sidebar" type="button" className="inline-flex items-center p-2 mt-2 ml-3 text-sm rounded-lg md:hidden focus:outline-none focus:ring-2 text-gray-400 hover:bg-gray-700 focus:ring-gray-600 absolute right-4 top-4" onClick={handleClick}>
            <HiOutlineMenuAlt1 className='w-6 h-6' />
         </button>

         <aside id="logo-sidebar" className={`fixed top-0 left-0 h-screen bg-gray-800 z-40 ease-in duration-300 w-[72vw] md:w-[20vw] ${navbar ? 'md:translate-x-0 -translate-x-0' : 'md:translate-x-0 -translate-x-full'}`} aria-label="Sidebar">
